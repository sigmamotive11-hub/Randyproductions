...
export default function HomePage() {
  ...
  return (
    <div>
      {/* ===== HERO ===== */}
      <section className="relative min-h-screen flex items-center overflow-visible pt-24 pb-12 md:pt-0 md:pb-0">
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-6 md:px-16 w-full flex flex-col lg:flex-row items-center min-h-screen lg:items-center gap-8 lg:gap-0">
          {/* Left: Text content */}
          <div className="flex-1 min-w-0 pt-8 pb-4 lg:pt-24 lg:pb-20 text-center lg:text-left order-2 lg:order-1">
            {/* Tag */}
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-[rgba(212,175,55,0.25)] mb-6 md:mb-8">
              <span className="w-2 h-2 rounded-full bg-[#d4af37] animate-pulse" />
              <span className="text-[#d4af37] text-xs font-semibold uppercase tracking-[3px]">By Randyy</span>
            </div>

            {/* Title */}
            <h1 className="text-4xl md:text-5xl lg:text-7xl font-black text-white uppercase leading-[0.9] mb-3 md:mb-4 tracking-tight">
              Randy
              <br />
            ...
            {/* Subtitle */}
            <p className="text-base md:text-xl lg:text-2xl text-[rgba(255,255,255,0.4)] font-light uppercase tracking-[4px] md:tracking-[6px] mb-6 md:mb-8">
              Next Gen Sound
            </p>

            {/* Description */}
            <p className="text-[rgba(255,255,255,0.5)] text-sm md:text-base lg:text-lg leading-relaxed mb-8 md:mb-10 max-w-[480px] mx-auto lg:mx-0">
            ...
            <div className="flex flex-wrap gap-3 md:gap-4 justify-center lg:justify-start">
              <button onClick={() => setView('beats')}
                className="group relative px-6 md:px-8 py-3 md:py-4 bg-[#d4af37] text-[#050505] text-xs md:text-sm font-extrabold uppercase tracking-[2px] rounded overflow-hidden transition-all duration-500 hover:shadow-[0_0_40px_rgba(212,175,55,0.3)]">
                <span className="relative z-10">Browse Catalog</span>
                <div className="absolute inset-0 bg-white translate-y-full group-hover:translate-y-0 transition-transform duration-500" />
              </button>
              <button onClick={() => setView('pricing')}
                className="px-6 md:px-8 py-3 md:py-4 bg-transparent text-white text-xs md:text-sm font-semibold uppercase tracking-[2px] rounded border border-[rgba(255,255,255,0.1)] hover:border-[#d4af37] hover:text-[#d4af37] transition-all duration-300">
                View Pricing
              </button>
            </div>

            {/* Quick stats */}
            <div className="flex gap-8 md:gap-10 mt-10 md:mt-14 pt-6 md:pt-8 border-t border-[rgba(255,255,255,0.05)] justify-center lg:justify-start">
              {[
                ...
                <div key={s.label}>
                  <p className="text-white text-xl md:text-2xl font-black">{s.num}</p>
                  <p className="text-[rgba(255,255,255,0.3)] text-[10px] md:text-xs uppercase tracking-wider mt-1">{s.label}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-1 items-center justify-center relative order-1 lg:order-2 w-full lg:h-screen lg:w-auto">
            <div className="relative w-[140px] h-[140px] sm:w-[200px] sm:h-[200px] md:w-[320px] md:h-[320px] lg:w-[400px] lg:h-[400px] flex items-center justify-center">
              {/* Gold glow behind logo */}
              <div
                className="absolute inset-0 rounded-full opacity-25 blur-[40px] lg:blur-[80px]"
                style={{
      ...
      <section className="py-16 md:py-24 px-6 md:px-16 relative">
        <div className="max-w-[1400px] mx-auto">
            ...
            {featuredBeats.map((beat, idx) => (
              <div key={beat.id}
                ...
                <div className="p-5 md:p-6">
                  <h3 className="text-white text-base md:text-lg font-bold uppercase tracking-wider mb-2">{beat.title}</h3>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[#d4af37] text-lg md:text-xl font-black">${beat.price}</span>
                    <button onClick={(e) => { e.stopPropagation(); handleBuy(beat); }}
                      className="px-4 md:px-5 py-2 md:py-2.5 bg-[rgba(212,175,55,0.1)] text-[#d4af37] border border-[rgba(212,175,55,0.2)] rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-[#d4af37] hover:text-[#050505] transition-all duration-300">
                      License
                    </button>
                  </div>
                </div>
          ...
          {featuredBeats.length === 0 && (
            <p className="text-[rgba(255,255,255,0.2)] text-center py-12">No beats uploaded yet.</p>
          )}
      ...
      {/* ===== LOOP KITS ===== */}
      <section className="py-16 md:py-24 px-6 md:px-16 relative" style={{ background: 'linear-gradient(180deg, transparent, rgba(212,175,55,0.02), transparent)' }}>
        <div className="max-w-[1400px] mx-auto">
          <div className="flex items-end justify-between mb-10 md:mb-14">
            <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
            {beats.filter(b => b.tags.includes('Loop Kit')).map(kit => (
              <div key={kit.id}
                className="group relative rounded-xl p-6 md:p-8 flex flex-col items-center justify-center min-h-[240px] cursor-pointer transition-all duration-500 hover:scale-[1.02]"
                style={{
                ...
                <h3 className="text-white text-lg md:text-xl font-bold uppercase tracking-wider mb-2">{kit.title}</h3>
      ...
      <section className="py-16 md:py-24 px-6 md:px-16">
        <div className="max-w-[900px] mx-auto text-center">
          <h2 className="text-2xl md:text-5xl font-black text-white uppercase tracking-tight mb-6">
            Ready to <span className="text-[#d4af37]">Level Up</span>?
          </h2>
          <p className="text-[rgba(255,255,255,0.4)] text-base md:text-lg mb-10 max-w-[500px] mx-auto">
